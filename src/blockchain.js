...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            //assign previousBlockHash if the block is not Genesis Block.
            if(self.height != -1){
            ...
            //push add to blockchain
            self.chain.push(block);
            //resolve the result
            if(block){
                resolve(block);
            } else {
                reject(new Error('error in _addBlock method in Blockchain class'));
            }
        });
    }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            self.chain.forEach((p) => {
                if(p.height > 0){ //to exclude genesis block
                    if(p.previousBlockHash != self.chain[p.height - 1].hash){
                        errorLog.push('hash reference problem at block:' + p.height);
                    }
                }
                //check block is valid, not tampered.
                p.validate().then((data) => {
